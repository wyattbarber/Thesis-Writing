...
One example of a grasping based mechanism is proposed by Chi et al., and shown in Fig. {fig:claw}. It aims 
...
One such device is that proposed by Hoang et al., shown in Fig. {fig:clasp}, which is intended for use in power line inspection. It uses ribbons to passively close an electromagnetic clasp once the drone has approached 
...
The device designed by Kominami et al., shown in Fig. {fig:passive}, is another grasping type device that grasps a perch beneath the drone. 
...
Fig. {fig:gripper} illustrates a device proposed by Roderick et al. which is designed to grip the bark of a tree \cite{roderick_bioinspired_2017}.
While it does not attach by grasping an actuator around the circumference of a tree like a typical grasping based device would, Fig. {fig:gripper}
...
One example of such a device is shown in Fig. {fig:electroadhesive}, designed by Gaule et al. It uses electrostatic adhesion to 
...
Kominami et al. designed a perch detection method in conjunction with their passive grasping device shown in Fig. {fig:passive}.
